...
def reset(event):
    for slider in sliders:
        slider.reset()
    if checkbox.get_status()[0]:  # якщо увімкнений, вимкнути
...
ax.set_title("Функція гармоніки з накладеним шумом")
ax.legend(["Шум", "Гармонічна крива"], loc='upper right')
line_with_noise.set_visible(False)

# Слайдери
s_amp = Slider(plt.axes([0.25, 0.35, 0.65, 0.03]), 'Амплітуда', 0.0, 2.0, valinit=init_amplitude)
s_freq = Slider(plt.axes([0.25, 0.30, 0.65, 0.03]), 'Частота', 0.0, 5.0, valinit=init_frequency)
s_phase = Slider(plt.axes([0.25, 0.25, 0.65, 0.03]), 'Фаза', 0.0, 2 * np.pi, valinit=init_phase)
s_nmean = Slider(plt.axes([0.25, 0.20, 0.65, 0.03]), 'Середнє Шуму', -1.0, 1.0, valinit=init_noise_mean)
s_ncov = Slider(plt.axes([0.25, 0.15, 0.65, 0.03]), 'Дисперсія Шуму', 0.0, 1.0, valinit=init_noise_cov)

sliders = [s_amp, s_freq, s_phase, s_nmean, s_ncov]  # масив слайдерів

# Чекбокс шуму
checkbox = CheckButtons(plt.axes([0.25, 0.05, 0.15, 0.04]), ['Показати Шум'], [False])

# Кнопка Reset
button = Button(plt.axes([0.8, 0.05, 0.1, 0.04]), 'Скинути')
...
checkbox.on_clicked(lambda val: update())
